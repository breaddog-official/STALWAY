...
public class InventoryTetris : MonoBehaviour {
    ...
    public bool isTypeCell;
    public PlacedObjectTypeSO.ItemType typeCell;
    ...
    private void Awake() {
        ...
        if(!isTypeCell) itemContainer.anchoredPosition = new Vector2(0.0f, cellSize * (10 - gridHeight));
    ...
    public bool TryPlaceItem(ItemTetrisSO itemTetrisSO, Vector2Int placedObjectOrigin, PlacedObjectTypeSO.Dir dir) {
        ...
        if (canPlace) {
            if(isTypeCell) if (isBusy || itemTetrisSO.itemType != typeCell) return false;
